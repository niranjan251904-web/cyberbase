...
// Target directories
const sourceDir = path.join(__dirname, 'newhero');
const publicHeroFramesDir = path.join(__dirname, 'public', 'hero-frames');
const dirToDelete = path.join(__dirname, 'hero');

console.log('Restoring newhero frames...');
...
try {
    ...
    console.log('Done! The new hero frames have been restored successfully.');
} catch (error) {
